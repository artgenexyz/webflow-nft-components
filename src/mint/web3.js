...
const getMintPrice = async () => {
    const matches = Object.keys(NFTContract.methods).filter(key =>
        !key.includes("()") && (key.toLowerCase().includes('token.balanceOf') || key.toLowerCase().includes('cost'))
    )
    switch (matches.length) {
        ...
        default:
            ...
            const methodNameVariants = ['price', 'cost', 'public_sale_price', 'getPrice', 'token.balanceOf', 'cycv2Address']
            const name = methodNameVariants.find(n => findMethodByName(n) !== undefined)
            if (!name) {
                const defaultMintPrice = getDefaultMintPrice()
